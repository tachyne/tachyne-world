...
# The advancement DATA is read from ADV_DATA, not the canonical version: 26.x
# rewrote the predicate format (single conditions, namespaced and renamed
# predicate fields, match_block, any_of terms…) and this distiller reads the
# 1.21.x shape. The tree itself barely moved — 26.3 adds husbandry/uh_oh and
# two biomes to Adventuring Time, both content the engine does not have yet,
# and taking the biomes early would make that advancement impossible. Names
# resolve to the canonical id space through itemnames_gen.go either way.
ADV_DATA = "1.21.11"
JAR = sys.argv[1] if len(sys.argv) > 1 else canon.jar(ADV_DATA)
...
def main():
    ...
    w.append(f"// Source: {ADV_DATA} server jar data/minecraft/advancement (recipe")
