...
def main():
    ...
    with open(OUT, "w") as f:
        ...
        f.write("// (EmeraldForItems + ItemsForEmeralds + EnchantedItemForEmeralds listings,\n")
        f.write("// in source order per tier; the remaining exotic types are omitted).\n\n")
        ...
        f.write("\t// kind is vTradeFixed for a plain offer, or vTradeEnchantedGear for a\n")
        f.write("\t// piece of equipment whose enchantments (and final price) are rolled\n")
        f.write("\t// when the villager unlocks it.\n")
        f.write("\tkind int32\n")
        f.write("}\n\n")
        f.write("const (\n")
        ...
        for pi in sorted(table):
            ...
            for tier in sorted(table[pi]):
                f.write(f"\t\t{tier}: {{")
                for (ii, ic, oi, oc, mu, xp, kind) in table[pi][tier]:
                    f.write(f"{{{ii}, {ic}, {oi}, {oc}, {mu}, {xp}, {kind}}}, ")
                f.write("},\n")
            f.write("\t},\n")
        f.write("}\n")
