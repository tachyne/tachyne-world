...
test, hardness, resistance, loot table, block entity, the block's item,
whether it needs the right tool and its sound type; for items, stack size,
durability, food and each tool's rules for which blocks it is correct for;
and every sound type's volume, pitch and sound events.
