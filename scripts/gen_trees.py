...
# Fallen trees (minecraft:fallen_tree): the stump/log split, with the shape
# of the decorators asserted — the Go side bakes their semantics in.
fallen_rows = {}
for name, c in sorted(fallen_feats.items()):
    log, _ = state_of(c["trunk_provider"])
    if log is None or log not in base_id:
        raise SystemExit(f"{name}: fallen trunk provider unresolved")
    lmin, lmax = rng_range(c["log_length"])
    sd = [x["type"].removeprefix("minecraft:") for x in c["stump_decorators"]]
...
L += ["// FallenTrees are the fallen-log features: a stump and a sideways run,",
