...
func TestFastBreakCheatReverted(t *testing.T) {
	...
	if minDigTicks(worldgen.Stone, 0, 0, 1) < 5 {
		t.Fatalf("stone-by-hand minimum should be many ticks, got %d", minDigTicks(worldgen.Stone, 0, 0, 1))
	}
	// A diamond pick is legitimately fast — the floor scales with the tool.
	if minDigTicks(worldgen.Stone, itemByName["diamond_pickaxe"], 0, 1) >= minDigTicks(worldgen.Stone, 0, 0, 1) {
		t.Fatal("tools must lower the floor")
	}
	if minDigTicks(worldgen.GrassBlock, 0, 0, 1) <= 0 {
