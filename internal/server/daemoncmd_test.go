...
func TestDaemonFleetCommand(t *testing.T) {
	...
	s.handleCommand(p, "daemon install webmap")
	...
	s.handleCommand(p, "daemon install github.com/x/mapd@v1.2.0")
	json.Unmarshal(stub.payload, &inst)
	...
	if !waitChatLine(p, "1 of 2 daemons outdated — /daemon upgrade <name> rolls the fleet progressively.") {
	...
	s.handleCommand(p, "daemon search map")
