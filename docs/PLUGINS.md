...
For a full **3D web map**, the bus is consumed from outside the daemon tier
entirely: [tachyne-map](https://github.com/tachyne/tachyne-map) is its own
pod that renders the world natively in Go — vanilla block models and
textures, face culling, baked light and biome tint — with no Java and no
intermediate save format. It reads the world read-only through the public
`worldread` package, asks the engine for its seed over this bus, and follows
`mc.event.block_change` so the map updates live. (It replaced a
BlueMap-based daemon that exported to the vanilla Anvil format and drove a
JVM renderer; that exporter survives standalone as `cmd/anvil-export` if you
want your world as a vanilla Minecraft save.)
