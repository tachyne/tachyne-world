...
| Mace (player) | smash attack + density/breach/wind_burst | `mace.go`: base 6, attack speed 0.6 (period 33); a hit while falling >1.5 blocks adds vanilla fall bonus (+4/blk to 3, +2/blk 3-8, +1/blk beyond) added after cooldown scale before crit ×1.5, negates the attacker's fall damage, and a 3.5-block shockwave shoves nearby mobs (2× if fell >5); density +0.5·lvl/blk, breach −0.15·lvl armour effectiveness (`hurtBreach`), wind_burst launches the attacker up (1.2/1.75/2.2) via a spinUntil grace | 🟡 (mob targets; PvP smash TBD) |
...
| Travel | portals: frame validation, 4s dwell, portal search + linking, spawn platform | full frame validation (2-21 interior, both axes), flint ignite, 80-tick dwell (creative ~instant), 8:1 scaling, PortalForcer search/create each trip (see Portal linking) | ✅ |
...
| Portal linking | 128-block search, forced platforms | PortalForcer port: every trip (players, mobs, items, arrows, TNT, falling blocks) searches portal blocks in a square of 16 (into the Nether) / 128 (out), closest then lowest, border-filtered; none → createPortal's spiral (r=16) for a frame spot with solid ground, full-width preferred, else the 3×2 obsidian platform at y 70..top-9; exit spot/heading from the relative position in the entry sheet (quarter turn across axes); findCollisionFreePosition not modelled | ✅ |
...
| Portal pairing | proximity re-search each trip | no pair registry: the search finds the portal each time, as vanilla's does | ✅ |
| Portal linking round-trip | 128-block overworld search | the search reads the edit overlay (portal blocks are always edits), so it never generates a chunk | ✅ |
