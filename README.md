...
| Vehicles | 🟡 | Boats in all nine woods and the bamboo raft, plus their chest variants (sneak-click opens the cargo, which spills when the boat breaks); minecarts on auto-shaping rails with vanilla's classic cart physics rolled by the server — gravity, slopes that speed a cart up and slow it down, powered rails that boost a rolling cart and brake it when unpowered, the station start off a solid block, activator rails that throw the rider off, and a rider whose keys only nudge a resting cart — though there are no chest/furnace/hopper/TNT carts yet, and a cart does not scoop up mobs. Parked boats and carts survive restarts, and they exist in every dimension — a boat launched on a Nether lake or a cart on an End rail behaves like an overworld one, and each player only sees the vehicles of the dimension they are in. |
