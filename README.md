...
| Structures | 🟡 | Villages (with per-profession loot chests), dungeons, mineshafts, strongholds, ruins, desert temples, ruined portals, pillager outposts, and deep-dark ancient-city vaults (reinforced-deepslate chambers with sculk, can-summon shriekers, and a chests/ancient_city loot chest). Structure chests fill from the real vanilla loot tables — weighted, with enchanted gear — deterministically per chest. No ocean monuments, mansions, shipwrecks or trial chambers yet; ancient cities are a compact stand-in, not the full jigsaw megastructure. |
