...
| Structures | 🟡 | Villages (with per-profession loot chests), dungeons, mineshafts, strongholds, ruins, desert temples, ruined portals, pillager outposts, deep-dark ancient-city vaults, ocean monuments (prismarine halls patrolled by guardians and elder guardians, gold-block treasure core), shipwrecks (supply/treasure/map chests), and beach buried treasure. Structure chests fill from the real vanilla loot tables — weighted, with enchanted gear — deterministically per chest. Current structures are hand-built stand-ins; a vanilla NBT-template pipeline (for the exact vanilla layouts) is in progress. No woodland mansions or trial chambers yet. |
